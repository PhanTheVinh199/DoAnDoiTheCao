...
{
    $donhang = DoithecaoDonhang::when($request->ma_don, function ($query) use ($request) {
        return $query->where('ma_don', 'like', '%' . $request->ma_don . '%');
    })->paginate(10); // 10 đơn hàng mỗi trang
    ...
    public function store(Request $request) {
        $request->validate([
            ...
            'thanhvien_id' => 'required|exists:thanhvien,id_thanhvien', // Add this line
